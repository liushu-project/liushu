...
use crate::{config::Config, dirs::MyProjectDirs, engine::state::State, error::LiushuError};
...
pub fn deploy(proj_dirs: &MyProjectDirs) -> Result<(), LiushuError> {
    ...
    for formula in &config.formulas {
        ...
        let new_dict_digest = formula.get_dictionaries_digest(&proj_dirs.config_dir);
        if old_dict_digest != new_dict_digest {
            formula.compile(proj_dirs)?;
